...
export function FieldHighlights() {
  ...
  return <div className="field field-sequence" ref={fieldRef}>
    ...
    <div className="field-sequence-heading"><p className="field-sequence-label">People you can learn from</p><span className="field-progress micro" aria-hidden="true">{`0${selected + 1} / 0${c.field.people.length}`}</span></div>
    <div className="field-list-scroll"><ul className="field-people">{c.field.people.map((person, i) => <li key={person} className={`field-person${i === selected ? " is-active" : ""}`}><button onClick={() => showPerson(i)} aria-current={i === selected ? "true" : undefined} aria-describedby={`field-copy-${i}`}><span className="micro" aria-hidden="true">0{i + 1}</span><span>{person}</span><span className="field-person-line" aria-hidden="true" /></button><p className="field-person-copy" id={`field-copy-${i}`}>{c.field.personNotes[i]}</p></li>)}</ul></div>
